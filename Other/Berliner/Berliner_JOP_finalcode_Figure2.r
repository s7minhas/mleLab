...
setwd("~/Dropbox/Duke/Spring 2015/PS 733/Other/Berliner") 
...
data08 = data[data$YEAR==2008,] # Subset to 2008, so we can get mean 2008 levels
vars = names(coef(m4)); vars=vars[-1] # Pull out coefficients used in model 4, take out intercept
ivs = intersect(vars, names(data08))
simData = apply(data08[,ivs], 2, function(x){ mean(x, na.rm=TRUE) }) # Calculate mean of every variable in 2008

# IMF dummy and NEW dem should be set at median (Berliner doesnt do this but you should)
simData['IMFDUMMY'] = median(data08$IMFDUMMY, na.rm=T)
simData['NEWDEM'] = median(data08$NEWDEM, na.rm=T)

# Add duration dummies
ddums = rep(0, 18); names(ddums)=paste0('dd',1:18)
simData = c(simData, ddums)
simData['dd18'] = 1 # Set duration dependent dummy 18 to one
...
tmp=tmp + scale_y_continuous(breaks=seq(0,1,.2), labels=seq(0,1,.2))
